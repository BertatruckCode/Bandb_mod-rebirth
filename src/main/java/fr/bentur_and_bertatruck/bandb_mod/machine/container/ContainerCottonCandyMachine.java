...
import fr.bentur_and_bertatruck.bandb_mod.machine.slot.SlotCottonCandyMachineStick;
import fr.bentur_and_bertatruck.bandb_mod.machine.slot.SlotCottonCandyMachineSugar;
...
public class ContainerCottonCandyMachine extends Container {
	private TileEntityCottonCandyMachine tileEntity;
	public int lastWorkTimeLeft;
	public int lastCurrentItemWorkTime;
	public int lastWorkTime;

	public ContainerCottonCandyMachine(InventoryPlayer inventory, TileEntityCottonCandyMachine tileentity) {
		this.tileEntity = tileentity;
		this.addSlotToContainer(new SlotSyrupMachineOutput(tileentity, 0, 147, 20)); //output 
		this.addSlotToContainer(new SlotSyrupMachineFuel(tileentity, 1, 8, 61));
		this.addSlotToContainer(new SlotCottonCandyMachineStick(tileentity, 2, 28, 20));
		this.addSlotToContainer(new SlotCottonCandyMachineSugar(tileentity, 3, 8, 20));
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int slotid) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			default: {
				 if (BandbFuel.getFuelValue(stack.getItem()) > 0 && stack.getItem() != Items.stick) {
					if (!this.mergeItemStack(stack, 1, 2, false))
						return null;
				} else if(stack.getItem() == BandbItems.itemCottonCandySugar) {
