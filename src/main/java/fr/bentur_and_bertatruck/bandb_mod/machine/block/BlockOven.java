...
public class BlockOven extends BlockContainer {
	...
	public void registerBlockIcons(IIconRegister iconRegister) {
		...
		this.iconFront = iconRegister.registerIcon(Bandb_mod.MODID+ (this.isActive ? ":gui/oven/furnace_front_on": ":gui/oven/furnace_front_off"));
	...
	public Item getItemDropped(World world, int x, int y, int z) {
		return Item.getItemFromBlock(BandbMachines.blockOvenIdle);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
		if (!world.isRemote) {
			FMLNetworkHandler.openGui(player, Bandb_mod.instance, BandbMachines.guiIDOven, world, x, y, z);
		}
		return true;
	}
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityplayer, ItemStack itemstack) {
		...
		if (itemstack.hasDisplayName()) {
			((TileEntityOven) world.getTileEntity(x, y, z))
					.setGuiDisplayName(itemstack.getDisplayName());
		}
	}

	public static void updateBlockOvenState(boolean active, World worldObj,
		...
		if (active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, BandbMachines.blockOvenActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, BandbMachines.blockOvenIdle);
		}
		keepInventory = false;
